...
int Add_Chk(char *sen) {
    ...
    while (_stricmp(Instr[k].instruct, Instr[0].instruct) || _stricmp(Instr[k].dest, Instr[0].dest) ||
        _stricmp(Instr[k].sour, Instr[0].sour) || _stricmp(Instr[k].word_type, Instr[0].word_type)) {
        k--;
    }
    return k;
}
...
void PassII(char*buf) {
    ...
    if (i) {
        ...
        if (!strcmp(Instr[i].dest, "r")) {
            while (_stricmp(Reg[j].reg_name, Sen.operand[0]))j++;
            strncpy(strchr(modInstr[InstrP].mod_reg, '?'), Reg[j].reg_num, 3);
            j = 0;
            if (!strcmp(Instr[i].sour, "r")) {
                while (_stricmp(Reg[j].reg_name, Sen.operand[1]))j++;
                strncpy(strchr(modInstr[InstrP].mod_reg, '?'), Reg[j].reg_num, 3);
            }
...
void Symbol_Print() {
    ...
    for (i = 0; i < MaxS; i++) {
        if (!strcmp(Symbol[i].word_type, "w")) {
            printf("%s\t%X\t\t%d\n", Symbol[i].lc, (Symbol[i].lc != atoi(Symbol[i].data) ? 1 : 0));
